...
enum GameStates {
  ...
  ///Reset
  ///After reset,  [GameController]
  ///goto  [none]
  reset,

  ///The falling blocks have reached the bottom, and the blocks are now being fixed in the game matrix
  ///Immediately after the completion of the fixation, the next block will be dropped.
  mixing,

  ///Erasing row
  ///After the elimination is completed, the falling task of the next block will immediately begin.
  clear,

  ///The blocks quickly fall to the bottom
  drop,
}
...
///Mixed with [_data] in the [build] method to form a new matrix
/// [_ mask] The width and height of the matrix are the same as [_data]
/// For any _mask [x, y]:
/// If the value is 0, it has no effect on [_data]
/// If the value is -1, it means that the line in [_data] is not displayed
/// If the value is 1, it means that the line in [_data] is highlighted
...
// _ current has been integrated into _data, so it is no longer needed
    _current = null;

// Check if the game is over, i.e. check if the first line has element 1
    if (_data[0].contains(1)) {
      reset();
      return;
    } else {
// The game is not over yet, start the next round of falling blocks
      _startGame();
    }
  }
/// traverse the table
